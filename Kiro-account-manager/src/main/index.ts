...
export async function ssoDeviceAuth(
  bearerToken: string,
  region: string = 'us-east-1',
...
async function kiroApiRequest<T>(
  ...
  console.log(
    `[Kiro API] Machine ID: ${machineId || 'undefined'} (account: ${accountMachineId ? 'yes' : 'no'})`
  )
...
async function getUsageLimitsRest(
  ...
  const machineId = accountMachineId || getCurrentMachineId()
  console.log(
    `[Kiro REST API] Machine ID: ${machineId || 'undefined'} (account: ${accountMachineId ? 'yes' : 'no'})`
  )
...
`
  ...
  ipcMain.handle(
    'account-get-models',
    async (_event, accessToken: string, region?: string, profileArn?: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, accessToken: string, region?: string) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, accessToken: string, subscriptionType?: string, region?: string) => {
      try {
        const result = await fetchSubscriptionToken(
          { accessToken, region: region || 'us-east-1' } as ProxyAccount,
          subscriptionType
        )
  ...
  ipcMain.handle(
    'auto-register:activate-outlook',
